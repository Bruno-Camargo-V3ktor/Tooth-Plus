...
pub fn PatientAnamneseTab(
    ...
    rsx! {
        div { class: "anamnese-cards-container",
            ...
            // Termo e Declaração Legal de Veracidade (Conforme Ficha Física de Referência)
            div { style: "background: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 10px; padding: 18px 22px; margin-bottom: 20px; text-align: center;",
                p { style: "font-size: 13.5px; font-weight: 600; color: #334155; margin: 0 0 6px 0;",
                    "\"Assino este declarando verdadeiras as informações ditas acima.\""
                }
                p { style: "font-size: 12px; color: #64748b; margin: 0;",
                    "Histórico médico e bucal registrado sob sigilo profissional no prontuário eletrônico Tooth-Plus."
                }
            }
            ...
            if can_write {
                div { style: "display: flex; justify-content: flex-end; gap: 10px; margin-bottom: 30px;",
                    button {
                        r#type: "button",
                        class: "btn-primary",
                        style: "font-size: 14px; padding: 10px 24px; font-weight: 700;",
                        disabled: is_saving(),
                        onclick: {
                            let t_c = tok.clone();
                            let p_c = pat_id.clone();
